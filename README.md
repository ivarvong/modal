...
| `manhattanhenge.exs`         | Auditable agent build pipeline: Claude Code runs live in a Modal sandbox, writes a single Skyfield/FastAPI `app.py` onto a fresh `Modal.Volume`, and the orchestrator independently smoke-tests it before `Modal.Function.deploy_asgi/2` serves the same Volume read-only — no copy-out, no rebake. The live URL is curl-verified against the published 2026 Manhattanhenge dates (May 28 & 29), 299.1° crossing times, and refraction-corrected apparent altitude; `/source` serves the generated app code, while the Claude transcript stays local to avoid publishing agent session details. |
