...
| `manhattanhenge.exs`         | Reproduce Manhattanhenge end-to-end, no human in the loop: Elixir boots a sandbox, runs the **Claude Code CLI live** to write a DE440/Skyfield calc + FastAPI app (uv/Python), sanity-checks that the calc finds the az=299.1° crossings on the published 2026 dates (May 28 @ 20:13 EDT, May 29 @ 20:12 EDT) at the correct **apparent** (refraction-corrected) altitude, reads the app back out, bakes it into an Image, `Modal.Function.deploy_asgi/2` to a persistent endpoint, then curl-verifies the live URL agrees. The careful bit: refraction (~0.5°) exceeds the Sun's radius (0.27°), so on May 28 the geometric center is *below* the horizon while the apparent disk is visibly up — geometric altitude would be wrong. |
