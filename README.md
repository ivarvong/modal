...
| `manhattanhenge.exs`         | Reproduce Manhattanhenge end-to-end, no human in the loop — Modal runs both the **build** and the **serve**, with a `Modal.Volume` as the handoff. A sandbox runs the **Claude Code CLI live** to write a single `app.py` (DE440/Skyfield calc + FastAPI, uv/Python) onto a mounted Volume (logging Claude's `total_cost_usd`); on sandbox exit the worker commits it; `Modal.Function.deploy_asgi/2` mounts the **same Volume read-only** and serves `app.py` off the base image — no copy-out, no rebake (deploy ~0.5s). Then curl-verifies the live URL reports the published 2026 dates (May 28 @ 20:13 EDT, May 29 @ 20:12 EDT) at the correct **apparent** (refraction-corrected) altitude — and `GET /source` serves the app's own code straight off the Volume. The careful bit: refraction (~0.5°) exceeds the Sun's radius (0.27°), so on May 28 the geometric center is *below* the horizon while the apparent disk is visibly up. |
