...
| `manhattanhenge.exs`         | Auditable agent build pipeline ([design note](guides/manhattanhenge_agent_pipeline.md)): Claude Code runs live in a Modal sandbox, writes a Skyfield/FastAPI `app.py` onto a fresh `Modal.Volume`, and the orchestrator independently smoke-tests it before `Modal.Function.deploy_asgi/2` serves the same Volume read-only. The live URL is curl-verified against the 2026 Manhattanhenge dates, 299.1° crossing times, and refraction-corrected apparent altitude; `/source` serves generated code, while the Claude transcript stays local. |
